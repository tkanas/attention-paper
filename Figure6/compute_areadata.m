% Compute cov and gain boundaries in a kE-for loop, save areas between curves

clear all

kI=0.2;
kEvec=0.2:0.01:1;
areavec=zeros(length(kEvec),1);
...
for kE=kEvec
    ...
    ddrI=drIvec(2)-drIvec(1);
    areavec(ki)=sum(Covbdry-Gainbdry)*ddrI;
    ki=ki+1
end
