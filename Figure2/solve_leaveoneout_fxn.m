...
function solve_leaveoneout_fxn(st,stend,r2dataeb,r2datashuf,r2datasurr)
...
%for ndi=1:6
    ...
    if r2dataeb==1
        ...
        sname=['loo_nunits_real' num2str(ndi) '.mat'];
        fprintf('Saving %d',ndi);
        save(sname,'-v7.3');
        if ndi>st
          dname=['loo_nunits_real' num2str(ndi-1) '.mat'];
          delete(dname);
        end
    end
    if r2datashuf==1
        ...
        sname=['loo_nunits_shuf' num2str(ndi) '.mat'];
        fprintf('Saving %d',ndi);
        save(sname,'-v7.3');
        if ndi>st
          dname=['loo_nunits_shuf' num2str(ndi-1) '.mat'];
          delete(dname);
        end
    end
    if r2datasurr==1
        ...
        sname=['loo_nunits_surr' num2str(ndi) '.mat'];
        fprintf('Saving %d',ndi);
        save(sname,'-v7.3');
        if ndi>st
          dname=['loo_nunits_surr' num2str(ndi-1) '.mat'];
          delete(dname);
        end
    end
end
