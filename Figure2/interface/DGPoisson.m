%function [samples,gammas,Lambda,joints2D,pmfs,hists,supports,covmatunatt,covmatatt,meansU,meansA]=DGPoisson(meansA,meansU,SigmaA,SigmaU,Nsamples,acc)
function [samples,gammas,Lambda,joints2D,pmfs,hists,supports]=DGPoisson(meansA,SigmaA,Nsamples,acc)
...
[pmfs,supports] = PoissonMarginals(meansA,acc);

% find paramters of DG
[gammas,Lambda,joints2D]=FindDGAnyMarginal(pmfs,SigmaA,supports);

%{
[V,D]=eig(Lambda);
while min(min(D))<0
    nright=length(meansA);
    unattrates=meansU; attrates=meansA;
    covmatunatt=SigmaU; covmatatt=SigmaA;
    sortneurgood_alltr;
    ssi=ordind(1:nright-1);
    covmatunatt=covmatunatt(ssi,ssi);
    covmatatt=covmatatt(ssi,ssi);
    meansU=meansU(ssi);
    meansA=meansA(ssi);
    [pmfs,supports] = PoissonMarginals(meansA,acc);
    [gammas,Lambda,joints2D]=FindDGAnyMarginal(pmfs,SigmaA,supports);
